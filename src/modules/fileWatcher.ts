...
function doUpload() {
	...
	const currentDownloadTasks = getRunningTransformTasks().filter((task) => task.transferType === TransferDirection.REMOTE_TO_LOCAL);
...
function createWatcher(watcherBase: string, watcherConfig: { files: false | string; autoUpload: boolean; autoDelete: boolean }) {
	...
	if (watcherConfig.autoUpload) {
		const handleWithPause = (uri: vscode.Uri) => {
			if (app.state.isPaused()) {
				logger.info(`Auto Transfer is paused, do nothing`);
				return;
			}
			return uploadHandler(uri);
		};
		watcher.onDidCreate(handleWithPause);
		watcher.onDidChange(handleWithPause);
	}

	if (watcherConfig.autoDelete) {
		const handleWithPause = (uri) => {
			if (app.state.isPaused()) {
				logger.info(`Auto Transfer is paused, do nothing`);
				return;
			}
		...
		watcher.onDidDelete(handleWithPause);
	}
}
